...
import { Drawer, Box, Stack, TextField, Tabs, Button, Tab, Typography, List, ListItem, ListItemButton, ListItemText, IconButton, Slider, createTheme, ThemeProvider, Autocomplete, Stepper, Step, StepLabel, Grid, Card, CardHeader, CardContent, Checkbox, FormGroup, FormControlLabel, Rating } from "@mui/material";
    ...
    return (
        ...
        <Layout>
            <ThemeProvider theme={theme}>
                <Box>
                    <div style={{ backgroundImage: 'url("https://images.pexels.com/photos/1833306/pexels-photo-1833306.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2")', backgroundSize: 'cover', height: '30vh', display: 'flex', alignItems: 'flex-end' }}>
                                            ...
                                            <Card variant="outlined" sx={{display:'flex', width:'100%'}} >
                                                ...
                                                <CardContent>
                                                    <FormGroup sx={{display:'flex', flexDirection:'row'}}>
                                                        <FormControlLabel control={<Checkbox readOnly checked={handleIntroduction(item, 0)}></Checkbox>} label="窗户" />
                                                        <FormControlLabel control={<Checkbox readOnly checked={handleIntroduction(item, 1)}></Checkbox>} label="阳台" />
                                                        <FormControlLabel control={<Checkbox readOnly checked={handleIntroduction(item, 2)}></Checkbox>} label="洗衣房" />
                                                    </FormGroup>
                    ...
                    {activeStep === 1 && (
                        <div>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'flex-end', padding: 4 }}>
                                <Button style={{ marginRight: 2 }} onClick={() => { setActiveStep(activeStep - 1) }}>上一步</Button>
                                <Button variant="contained" onClick={() => { setActiveStep(activeStep + 1) }} >下一步</Button>
                            </div>
                        </div>
                    )}
                    {activeStep === 2 && (
                        <div>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'flex-end', padding: 4 }}>
                                <Button style={{ marginRight: 2 }} onClick={() => { setActiveStep(activeStep - 1) }}>上一步</Button>
                                <Button variant="contained" onClick={() => { }} >提交订单</Button>
                            </div>
                        </div>
                    )}
